...
class ThalliumUrl {
 public:
  ThalliumUrl(const std::string& protocol, const std::string& domain,
...
class ThalliumClient : public Client {
 public:
  explicit ThalliumClient(const std::string& addr,
                          const std::string& protocol = "ofi+sockets",
                          int port = 8200, const std::string& domain = "")
      : addr_(addr),
        protocol_(protocol),
        port_(port),
        domain_(domain),
...
class ThalliumServer : public Server {
 public:
  explicit ThalliumServer(const std::string& addr,
                          const std::string& protocol = "ofi+sockets",
                          int port = 8200, const std::string& domain = "")
      : addr_(addr),
        protocol_(protocol),
    ...
    try {
      ...
      throw std::runtime_error(
          std::string("ThalliumServer failed to initialize with addr '") +
          addr + "': " + e.what());
    }
  }

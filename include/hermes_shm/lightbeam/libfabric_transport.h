...
inline std::string addr_to_hex(const void* addr, size_t len) {
...
inline std::vector<uint8_t> hex_to_addr(const std::string& hex) {
...
public:
    LibfabricClient(const std::string &server_addr_hex, const std::string &protocol = "tcp", int port = 9222)
        ...
        std::vector<uint8_t> server_addr = hex_to_addr(server_addr_hex);
        std::cout << "[LibfabricClient] server_addr_hex=" << server_addr_hex << ", size=" << server_addr.size() << "\n";
        fi_addr_t server_fi_addr;
        ret = fi_av_insert(av_, server_addr.data(), 1, &server_fi_addr, 0, nullptr);
        std::cout << "[LibfabricClient] fi_av_insert ret=" << ret << ", server_fi_addr=" << server_fi_addr << "\n";
...
public:
    LibfabricServer(const std::string &addr, const std::string &protocol = "tcp", int port = 9222)
        ...
        addr_hex_ = addr_to_hex(addr_buf, addrlen);

...
struct Mutex {
  ...
  /** Default constructor */
  HSHM_INLINE_CROSS_FUN Mutex() : lock_(0) {}

  /** Copy constructor */
  HSHM_INLINE_CROSS_FUN Mutex(const Mutex &other) {}

  /** Explicit initialization */
  HSHM_INLINE_CROSS_FUN void Init() {
    lock_ = 0;
  }

  /** Acquire lock */
  HSHM_INLINE_CROSS_FUN void Lock(uint32_t owner) {
    do {
      for (int i = 0; i < 1; ++i) {
  ...
  HSHM_INLINE_CROSS_FUN bool TryLock(uint32_t owner) {
    if (lock_.load() != 0) {
      return false;
    }
...
struct ScopedMutex {
  ...
  /** Release the mutex */
  HSHM_INLINE_CROSS_FUN ~ScopedMutex() {
    Unlock();
  }

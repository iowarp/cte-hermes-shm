...
struct FreeListSetIpc : public ShmContainer {
  ...
  hipc::atomic<hshm::min_i32> rr_free_;
  hipc::atomic<hshm::min_i32> rr_alloc_;
...
struct FreeListSet {
  hipc::vector<hipc::pair<Mutex*, iqueue<MpPage>*>> lists_;
  hipc::atomic<hshm::min_i32> *rr_free_;
  hipc::atomic<hshm::min_i32> *rr_alloc_;
};
...
class ScalablePageAllocator : public Allocator {
 private:
  ...
  hipc::vector<FreeListSet> free_lists_;
 ...
 public:
  ...
  HSHM_INLINE_CROSS_FUN void CacheFreeLists() {
    ...
    for (FreeListSetIpc &free_list_set_ipc : *free_lists) {
      ...
      hipc::vector<hipc::pair<Mutex*, iqueue<MpPage>*>> &lists =
 ...
 private:
  ...
  HSHM_INLINE_CROSS_FUN MpPage* CheckLocalCaches(size_t size_mp, size_t exp) {
    ...
    if (size_mp <= max_cached_size_) {
      ...
      hipc::pair<Mutex*, iqueue<MpPage>*> &free_list_pair =
        free_list_set.lists_[conc];
      Mutex &lock = *free_list_pair.GetFirst();
      iqueue<MpPage> &free_list = *free_list_pair.GetSecond();

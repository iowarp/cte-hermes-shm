...
#ifndef HERMES_DATA_STRUCTURES__mpsc_fifo_list_queue_H
#define HERMES_DATA_STRUCTURES__mpsc_fifo_list_queue_H
...
/** forward pointer for mpsc_fifo_list_queue */
template <typename T, HSHM_CLASS_TEMPL_WITH_DEFAULTS>
class mpsc_fifo_list_queue;

/**
 * MACROS used to simplify the mpsc_fifo_list_queue namespace
 * Used as inputs to the HIPC_CONTAINER_TEMPLATE
 * */
#define CLASS_NAME mpsc_fifo_list_queue
...
class mpsc_fifo_list_queue : public ShmContainer {
 public:
  HIPC_CONTAINER_TEMPLATE((CLASS_NAME), (CLASS_NEW_ARGS))
  AtomicOffsetPointer head_shm_, tail_shm_;
 ...
 public:
  ...
  /** Constructor. Default. */
  HSHM_CROSS_FUN
  mpsc_fifo_list_queue() {
    shm_init(HERMES_MEMORY_MANAGER->GetDefaultAllocator<AllocT>());
  }

  /** Constructor. Int */
  HSHM_CROSS_FUN
  explicit mpsc_fifo_list_queue(size_t depth) {
    shm_init(HERMES_MEMORY_MANAGER->GetDefaultAllocator<AllocT>());
  }

  /** SHM constructor. Default. */
  HSHM_CROSS_FUN
  explicit mpsc_fifo_list_queue(const hipc::CtxAllocator<AllocT> &alloc) {
    shm_init(alloc);
  }

  /** SHM constructor. Int */
  HSHM_CROSS_FUN
  mpsc_fifo_list_queue(const hipc::CtxAllocator<AllocT> &alloc, size_t depth) {
    shm_init(alloc);
  }
  ...
  /** Copy constructor */
  HSHM_CROSS_FUN
  explicit mpsc_fifo_list_queue(const mpsc_fifo_list_queue &other) {
  ...
  HSHM_CROSS_FUN
  explicit mpsc_fifo_list_queue(const hipc::CtxAllocator<AllocT> &alloc,
                                const mpsc_fifo_list_queue &other) {
    init_shm_container(alloc);
    shm_strong_copy_op(other);
  }
  ...
  HSHM_CROSS_FUN
  mpsc_fifo_list_queue &operator=(const mpsc_fifo_list_queue &other) {
    if (this != &other) {
      shm_destroy();
      shm_strong_copy_op(other);
    }
  ...
  HSHM_CROSS_FUN
  void shm_strong_copy_op(const mpsc_fifo_list_queue &other) {
  ...
  /** Move constructor. */
  HSHM_CROSS_FUN
  mpsc_fifo_list_queue(mpsc_fifo_list_queue &&other) noexcept {
  ...
  HSHM_CROSS_FUN
  mpsc_fifo_list_queue(const hipc::CtxAllocator<AllocT> &alloc,
                       mpsc_fifo_list_queue &&other) noexcept {
    init_shm_container(alloc);
  ...
  HSHM_CROSS_FUN
  mpsc_fifo_list_queue &operator=(mpsc_fifo_list_queue &&other) noexcept {
    if (this != &other) {
      shm_destroy();
      if (this != &other) {
        memcpy((void *)this, (void *)&other, sizeof(*this));
  ...
  /** Check if the mpsc_fifo_list_queue is null */
  HSHM_CROSS_FUN
  bool IsNull() { return false; }

  /** Set the mpsc_fifo_list_queue to null */
  HSHM_CROSS_FUN
  void SetNull() {}
  ...
  qtok_t enqueue(const FullPtr<T> &entry) {
    ...
    // Our tail is updated
    do {
      OffsetPointer tail_shm = tail_shm_;
      if (tail_shm.IsNull()) {
        head_id = head_.load();
        if (head_id == tail_id) {
  ...
  /** Construct an element at \a pos position in the mpsc_fifo_list_queue */
  HSHM_CROSS_FUN
  ...
  qtok_t dequeue(FullPtr<T> &val) {
    ...
    if (cur_size == 0 || head_shm_.IsNull()) {
  ...
  /** Destroy all elements in the mpsc_fifo_list_queue */
  HSHM_CROSS_FUN
  void clear() {
    while (size()) {
      dequeue();
  ...
  /** Get the number of elements in the mpsc_fifo_list_queue */
  HSHM_CROSS_FUN
  size_t size() const {
...
using mpsc_fifo_list_queue =
    hshm::ipc::mpsc_fifo_list_queue<T, HSHM_CLASS_TEMPL_ARGS>;
...
#endif  // HERMES_DATA_STRUCTURES__mpsc_fifo_list_queue_H

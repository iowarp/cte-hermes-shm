...
#define HSHM_DATA_STRUCTURES_TEMPLATE_BASE(NS, HSHM_NS, ALLOC_T)               \
  ...
  using HSHM_NS::chararr;                                                      \
                                                                               \
  template <typename T> using iqueue = HSHM_NS::iqueue<T, ALLOC_T>;            \
                                                                               \
  template <typename T> using list = HSHM_NS::list<T, ALLOC_T>;                \
                                                                               \
  template <typename FirstT, typename SecondT>                                 \
  using pair = HSHM_NS::pair<FirstT, SecondT, ALLOC_T>;                        \
                                                                               \
  template <typename T> using spsc_queue = HSHM_NS::spsc_queue<T, ALLOC_T>;    \
  template <typename T> using mpsc_queue = HSHM_NS::mpsc_queue<T, ALLOC_T>;    \
  template <typename T>                                                        \
  ...
  using fixed_mpmc_ptr_queue = HSHM_NS::fixed_mpmc_ptr_queue<T, ALLOC_T>;      \
                                                                               \
  template <typename T> using slist = HSHM_NS::slist<T, ALLOC_T>;              \
                                                                               \
  template <typename T>                                                        \
  ...
  using unordered_map = HSHM_NS::unordered_map<Key, T, Hash, ALLOC_T>;         \
                                                                               \
  template <typename T> using vector = HSHM_NS::vector<T, ALLOC_T>;            \
                                                                               \

...
int main() {
  // Pretest
  ServerPretest<HSHM_DEFAULT_ALLOC_T>();

  // Create thallium server
  server_ = std::make_unique<tl::engine>(tcnst::kServerName,
                                         THALLIUM_SERVER_MODE, true, 1);
  std::cout << "Server running at address " << server_->self() << std::endl;

  // Test transfer of 0-length string
  auto string_test0 = [](const request &req, hipc::string &text) {
    bool ret = (text == "");
    req.respond(ret);
  };
  ...
  auto string_test1 = [](const request &req, hipc::string &text) {
  ...
  auto charbuf_test0 = [](const request &req, hshm::charwrap &text) {
    bool ret = (text == "");
    req.respond(ret);
  };
  ...
  auto charbuf_test1 = [](const request &req, hshm::charwrap &text) {
    bool ret = (text == tcnst::kTestString);
    req.respond(ret);
  };
  ...
  auto vec_of_int0_test = [](const request &req, hipc::vector<int> &vec) {
    bool ret = vec.size() == 0;
    req.respond(ret);
  };
  ...
  auto vec_of_int_large_test = [](const request &req, hipc::vector<int> &vec) {
  ...
  auto shm_ar_test = [](const request &req,

...
int main() {
    std::string url = "tcp://127.0.0.1:8192";
    int num_clients = 4;
    int num_iters = 10;

    auto server_ptr = TransportFactory::GetServer(url, Transport::kZeroMq);
    auto client_ptr = TransportFactory::GetClient(url, Transport::kZeroMq);

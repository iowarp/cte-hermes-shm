...
class SingletonGenerator:
    ...
    def _GenerateCC(self, path, h_file):
        ...
        for defn in self.defs:
            ...
            lines.append(f"template<> {defn.class_name} scs::GlobalSingleton<{defn.class_name}>::obj_ = {defn.class_name}();")
            #lines.append(f"template<> hermes_shm::Mutex scs::GlobalSingleton<{defn.class_name}>::lock_ = hermes_shm::Mutex();")
    ...
    def _GenerateH(self, path):
        ...
        lines.append("")
        for defn in self.defs:
            lines.append(f"#define {defn.macro_name} scs::GlobalSingleton<{defn.class_name}>::GetInstance()")
